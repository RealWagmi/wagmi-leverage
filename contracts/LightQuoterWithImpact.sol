...
import { SwapMath, FullMath } from "./vendor0.8/uniswap/SwapMath.sol";
import { BitMath } from "./vendor0.8/uniswap/BitMath.sol";
import { AmountsLiquidity } from "./libraries/AmountsLiquidity.sol";

contract LightQuoterV3WithImpact is ILightQuoterV3WithImpact {
    ...
    uint256 public immutable impact;

    constructor(uint256 _impact) {
        require(_impact > 0 && _impact < 100, "LtQV3:impact is bad");
        impact = _impact;
    }
    ...
    function quoteExactInputSingle(
        ...
        address swapPool,
        uint256 amountIn
    ) external view returns (bool impactReached, uint160 sqrtPriceX96After, uint256 amountOut) {
        ...
        impactReached = sqrtPriceX96After == cache.sqrtPriceX96Limit;
    }
    ...
    function quoteExactOutputSingle(
        ...
        address swapPool,
        uint256 amountOut
    ) external view returns (bool impactReached, uint160 sqrtPriceX96After, uint256 amountIn) {
        SwapCache memory cache = _prepareSwapCache(zeroForIn, swapPool);
        uint256 out;
